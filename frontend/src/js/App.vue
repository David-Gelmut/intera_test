<template>
  <div class="flex min-h-screen flex-col bg-slate-50 font-sans text-slate-800 antialiased">
    ...
    <div
        ...
        :class="{ 'lg:pl-64': authStore.isAuthenticated }"
    >
      <!-- Контент страницы -->
      ...
      <footer class="mt-auto border-t border-slate-200 bg-white py-6 px-4 md:px-8 text-center sm:text-left">
        <div class="max-w-7xl w-full mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs md:text-sm text-slate-500">
          <div>
            <p class="font-semibold text-slate-700">&copy; {{ currentYear }} Intera Maps. Все права защищены.</p>
          </div>
...
<!--<template>
  <div class="flex min-h-screen bg-slate-50 font-sans text-slate-800 antialiased">
    ...
    &lt;!&ndash; Боковая навигация (Sidebar) — показывается только авторизованным юзерам &ndash;&gt;
    <aside
        v-if="authStore.isAuthenticated"
    ...
    >
      ...
      <nav class="flex-1 space-y-1 px-4 py-6">
        <router-link
            ...
            active-class="!bg-blue-50 !text-blue-700 !font-semibold"
        >
          &lt;!&ndash; Универсальная SVG иконка папки вместо эмодзи &ndash;&gt;
          <svg class="mr-3 h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
          </svg>
          {{ route.name }}
      ...
      <div class="border-t border-slate-100 p-4 space-y-3">
        &lt;!&ndash; Блок Профиля (ссылка) &ndash;&gt;
        <router-link
            to="/profile"
            class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 transition-colors group"
            active-class="!bg-slate-50"
        >
          &lt;!&ndash; Круглый аватар пользователя &ndash;&gt;
          <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-100 font-semibold text-sm text-blue-700 uppercase tracking-wider group-hover:bg-blue-200 transition-colors">
            {{ userInitials }}
          </div>
          &lt;!&ndash; Имя и Email &ndash;&gt;
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-slate-900 truncate group-hover:text-blue-600 transition-colors">
              {{ authStore.user?.name || 'Пользователь' }}
            </p>
    ...
    &lt;!&ndash; Основной контейнер контента &ndash;&gt;
    <div
        class="flex flex-1 flex-col transition-all duration-300"
        :class="{ 'pl-64': authStore.isAuthenticated }"
    >
      <main class="flex-1 p-6 md:p-8 max-w-7xl w-full mx-auto">
        &lt;!&ndash; Анимация плавного появления страниц при роутинге &ndash;&gt;
        <router-view v-slot="{ Component }">
          <transition
              name="fade"
              mode="out-in"
